 ...
 *
 * 原理:图源层(sources/*.ts 的 fetchPage)是纯「请求+解析」,不依赖插件环境,
 * 因此可以直接在 node 里(经 tsx 运行)复用与插件运行时完全相同的代码路径,
 * 检测结果与插件内一致。
 * 插件内入口:顶栏「更多」→「图源」Tab 的「图源检测」按钮。
 */
import { checkSource, HealthStatus } from "../src/sources/health-check";
import { imageSources } from "../src/sources/registry";
...
console.log(
  `开始检测 ${imageSources.length} 个图源${keywordOverride ? `(关键字: ${keywordOverride})` : ""}...\n`,
);
...
const results = await Promise.all(
  imageSources.map(async (source) => {
    ...
    console.log(
      `${STATUS_MARK[result.status]} ${result.label}${tag}: ${result.count} 张, 耗时 ${(result.durationMs / 1000).toFixed(1)}s${detail}`,
    );
    return result;
  }),
);
...
results.sort(
  (a, b) => Number(b.enabled) - Number(a.enabled) || (order[a.status] ?? 9) - (order[b.status] ?? 9),
);

console.log("\n—— 检测报告 ——");
console.table(
  results.map(({ label, enabled, status, count, durationMs, error }) => ({
    图源: label,
    上架: enabled ? "✓" : "下架",
...
if (enabledFailed.length > 0) {
  console.log(
    `⚠️  ${enabledFailed.length} 个已上架图源异常: ${enabledFailed.map((result) => result.label).join("、")}`,
  );
  process.exitCode = 1;
} else {
