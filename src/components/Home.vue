<script>
import useEmoticons from "../composables/useEmoticons.js";
import ImageSourceSwitcher from "./ImageSourceSwitcher.vue";
import MoreDrawer from "./MoreDrawer.vue";
import DonateDrawer from "./DonateDrawer.vue";
import ImageList from "./ImageList.vue";
import ImageStarList from "./ImageStarList.vue";
import {ref} from "vue";
...
export default {
  name: "Home",
  components: {ImageStarList, ImageList, ImageSourceSwitcher, MoreDrawer, DonateDrawer},
  setup() {
    ...
    const {
      ...
      reload,
      loadMore,
    } = useEmoticons(() => imageStarList.value?.close())
    ...
    <n-layout-header style="height: 45px;" bordered>
      <n-space justify="space-between" align="center" size="small" style="height:100%;padding:0 5px">
        ...
        <n-space align="center" size="small">
          ...
          <MoreDrawer/>
          <!-- 圆形图标分页按钮 -->
          <n-button title="上一页" circle size="small" :focusable="false" :disabled="loading||!pagination.hasLess.value"
                    @click="previousPage">
          ...
          <n-button title="下一页" circle size="small" :focusable="false" :disabled="loading||!pagination.hasMore.value"
                    @click="nextPage">
            <svg class="page-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"
                 stroke-linecap="round" stroke-linejoin="round">
      ...
      <n-layout content-style="padding: 5px 10px;" :native-scrollbar="false" @scroll="loadMore">
        <n-spin :show="loading" style="min-height: 300px" description="努力加载中~">
          ...
          <div v-if="emoticons.length && pagination.hasMore.value" class="load-more">
            <n-button size="large" dashed :loading="loading" :focusable="false" class="load-more-btn"
