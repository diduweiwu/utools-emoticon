...
import useSourceHealthCheck from "@/composables/use-source-health-check";
import { fetchSourceSettings, isSourceEffective, saveSourceSettings } from "@/composables/use-config";
...
const isEnabled = (row: HealthCheckResult): boolean => isSourceEffective(row, fetchSourceSettings());

/** 切换某个图源开关(至少保留一个生效图源) */
const toggleSource = (row: HealthCheckResult, on: boolean): void => {
  const next = { ...fetchSourceSettings(), [row.id]: on };
  if (!on && imageSources.filter((source) => isSourceEffective(source, next)).length === 0) {
...
const columns: DataTableColumns<HealthCheckResult> = [
  {
    title: "图源",
    key: "label",
    width: 150,
    render(row) {
      return h("span", [
        ...
        !row.enabled && row.status !== HealthStatus.OK
          ? h(
              NTag,
              { size: "tiny", type: "info", round: true, style: "margin-left: 6px" },
              { default: () => "已下架" },
            )
          : null,
        // 补充说明(如 VIP 限制)
  ...
  },
  {
    ...
    render(row) {
      if (row.error) {
        return h(
          NText,
          { type: "error", style: "font-size: 12px; word-break: break-all" },
          { default: () => row.error },
        );
      }
  ...
  },
  {
    ...
    render(row) {
      ...
      return h(NSwitch, {
        size: "small",
        value: isEnabled(row),
        onUpdateValue: (value: boolean) => toggleSource(row, value),
      });
    },
  },
];
