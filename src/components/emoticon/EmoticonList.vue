...
import EmoticonViewer from "./EmoticonViewer.vue";
...
// 大图预览抽屉(暴露 show 方法,供中键点击打开)
const viewer = ref<InstanceType<typeof EmoticonViewer>>();

/** 中键点击图片时,打开从该图片开始的大图预览 */
const openViewer = (event: MouseEvent, index: number): void => {
  viewer.value?.show(event, props.emoticons, index);
};
</script>

<template>
  <div v-if="props.emoticons.length">
    <div class="emoji-grid" :style="{ '--emoji-size': `${props.width}px` }">
      <div
        v-for="(em, index) in props.emoticons"
        :key="index"
        class="emoji-pic"
        :style="{
          width: `${props.width}px`,
          height: `${props.height}px`,
          borderColor: checkIfCollected(em.imgSrc) ? 'orange' : 'lightgray',
        }"
      >
        <EmoticonItem :em="em" style="width: 100%; height: 100%" @click.middle="openViewer($event, index)" />
      </div>
    </div>
  </div>
  <div v-else>
    <n-empty v-if="!props.loading" size="huge">
      <template #icon>
        <svg
          t="1675996717002"
          class="icon"
          viewBox="0 0 1576 1024"
          version="1.1"
