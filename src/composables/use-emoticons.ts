...
import { DEFAULT_SEARCH_KEYWORD } from "@/sources/registry";

import { fetchActiveSource } from "./use-config";
import { downloadImages } from "./use-download";
...
type PaginationRefs = ToRefs<PaginationState>;

/**
 * 注册插件生命周期:副输入框、外部进入、回车搜索、退出清理。
 * 必须等平台 API 就绪后再执行 —— 模拟器/开发模式下注入晚于页面脚本。
 */
function setupPluginLifecycle(keyWord: Ref<string>, reload: () => void): void {
  /**
   * 注册副输入框(幂等,可安全重复调用)。
   * 首次加载插件时,本次调用可能早于平台的插件进入流程而被忽略,
   * 因此 onPluginEnter / onPluginReady 触发时还会各补挂一次。
   */
  const attachSubInput = (): void => {
    setSubInput(({ text }) => {
      keyWord.value = text;
    }, SUB_INPUT_HINT);
  };
 ...
 * 图源的增减全部发生在 sources/registry.ts,这里零改动(开闭原则)。
 *
 * @param reloadCallback 每次重新加载前的回调(如关闭收藏夹抽屉)
 */
export default function useEmoticons(reloadCallback?: () => void) {
  const { warning } = useMessage();
  const emoticons = ref<Emoticon[]>([]);
  const keyWord = ref("");
  const loading = ref(false);
  const pagination = toRefs(
    reactive<PaginationState>({
      pageNum: 1,
      pageSize: 20,
      hasMore: false,
      hasLess: false,
  ...
  const appendEmoticons = (items: Emoticon[]): void => {
    nextTick(() => emoticons.value.push(...items)).then(() => setTimeout(() => (loading.value = false), 100));
  };
   ...
   */
  const loadData = (pagination: PaginationRefs, options: { isAppend?: boolean } = {}): void => {
    ...
    const keyword = keyWord.value || source.defaultKeyword || DEFAULT_SEARCH_KEYWORD;
  ...
  /** 翻到下一页 */
  const nextPage = (options: { isAppend?: boolean } = {}): void => {
    pagination.pageNum.value += 1;
  ...
  /** 滚动到底部时,追加加载下一页 */
  const loadMore = (event: Event): void => {
