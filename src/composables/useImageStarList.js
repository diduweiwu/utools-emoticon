...
import {downloadCollectedImages} from "./useDownload.js";
import {composeCollectedFilePath, removeFile, whenPlatformReady} from "../platform/index.js";
...
function loadStarMap() {
    const stored = loadJsonStorage(STAR_LIST_KEY);
...
const starEmojiList = computed(() => {
    const imageLinks = Object.entries(starMap.value).map(([imgSrc]) => imgSrc);
    // 进收藏夹时尝试重新下载缺失的本地文件
    downloadCollectedImages(imageLinks);
    return Object.entries(starMap.value).map(([imgSrc]) => ({
        imgSrc,
 ...
 * 收藏夹
 */
export default function useImageStarList() {
    const {success, warning} = useMessage();

    /** 是否已收藏 */
    const checkIfCollected = (imgSrc) => !!starMap.value[imgSrc];
